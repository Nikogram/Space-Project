	...
	public FactionMap(Vec2f placePlayer, Coor coorPlayer, WarMap map, TextureManager textureManager) {
		...
		for (int j = 4; j > -1; j--) {
			for (int i = 0; i < 5; i++) {
				...
				carte[i][j] = new Sprite(placePlayer.getAdd(Gdx.graphics.getWidth() / 2 - 100 + 21 * i, Gdx.graphics.getHeight() / 2 - 100 + 21 * j), new Vec2f(0, 0),
						textureManager.getTexture(translate(coorPlayer.addXY(i - 2, j - 2), map)));
			}
		}
