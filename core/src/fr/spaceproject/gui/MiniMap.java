		...
		for (int i = 0; i < station.getSize().x; i++)
			for (int j = 0; j < station.getSize().y; j++) {
				if (station.moduleIsShown(new Vec2i(i, j)))
					stationInt[i][j] = new Sprite(
							Conversion(playerPosition, station.getPosition().getAdd((i + 0.66f - (float) station.getSize().x / 2) * 140, (j + 0.80f - (float) station.getSize().y / 2) * 140)),
							new Vec2f(0, 0), textureManager.getTexture("StationUselessMiniMap"));
			}
		return stationInt;
	}
