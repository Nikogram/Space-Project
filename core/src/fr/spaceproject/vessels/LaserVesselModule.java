		...
		if (!laserIsAlreadyUpdate) {
			...
			for (int x = 0; x < station.getSize().x; ++x) {
				for (int y = 0; y < station.getSize().y; ++y) {
					if (station.getModuleType(new Vec2i(x, y)).ordinal() > VesselModuleType.Broken.ordinal()
							&& station.getModuleSprite(new Vec2i(x, y), false).isCollidedWithSegment(vertex1, vertex2)) {
					...
					if (tempSprite.isCollidedWithSegment(vertex1, vertex2))
						for (int x = 0; x < vessels.get(i).getSize().x; ++x) {
							for (int y = 0; y < vessels.get(i).getSize().y; ++y) {
								...
								if (vessels.get(i).getModuleType(new Vec2i(x, y)).ordinal() > VesselModuleType.Broken.ordinal()
										&& vessels.get(i).getModuleSprite(new Vec2i(x, y), false).isCollidedWithSegment(vertex1, vertex2)) {
									float distance = getSpritePosition().getDistance(vessels.get(i).getModulePosition(new Vec2i(x, y))) - getSpriteSize().x / 2
											- vessels.get(i).getModuleSize(new Vec2i(x, y)).x / 2;
