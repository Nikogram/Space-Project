		...
		if (!isDestroyed) {
			...
			for (int x = 0; x < modules.length; ++x) {
				for (int y = 0; y < modules[x].length; ++y) {
					((StationModule) modules[x][y]).update(lastFrameTime, new Sprite(cockpitPositionPixels, new Vec2f(140, 140), textureManager.getTexture("Blank")),
							new Vec2i(x - cockpitPosition.x, y - cockpitPosition.y), actions, vessels, faction, factionsAgressivity);
