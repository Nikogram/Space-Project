...
public class Background
{
	...
	Background(Vec2f sectorSize, TextureManager textureManager)
	{
		...
		for (int i = 0; i < starsSprites.length; ++i)
		{
			...
			starsSprites[i] = new Sprite(new Vec2f(2 * (float)(Math.random() * 2 * sectorSize.x - sectorSize.x), 2 * (float)(Math.random() * 2 * sectorSize.y - sectorSize.y)), new Vec2f(starSize, starSize), textureManager.getTexture("Blank"));
		}
	}
